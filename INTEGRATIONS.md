...
```
Your LLM  ──tool_call──▶  sulcus  ──SQL──▶  PGlite/Postgres-compatible backend (local)
                ◀──result─────────────────────────────────────────
```
...
proc = subprocess.Popen(["./target/debug/sulcus", "serve"],
                        stdin=subprocess.PIPE, stdout=subprocess.PIPE, text=True, bufsize=1)
...
// Start SULCUS sidecar
const proc = spawn("./target/debug/sulcus", ["serve"], {
...
cargo run -p sulcus -- serve-http --bind 127.0.0.1:8080
```
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize"}' | ./sulcus serve
echo '{"jsonrpc":"2.0","id":2,"method":"tools/list"}' | ./sulcus serve
echo '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"add_memory","arguments":{"content":"hello world"}}}' | ./sulcus serve
```
...
Sulcus integrates with [OpenClaw](https://github.com/openclaw/openclaw) as a full memory backend plugin. This replaces OpenClaw's default file-based memory with Sulcus's reactive, thermodynamic memory engine, giving your agents heat-based decay, cross-agent sync, and programmable triggers.
