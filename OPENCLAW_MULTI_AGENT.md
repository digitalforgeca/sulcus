...
* **The Edge (Local Sidecar):** Each OpenClaw instance runs its own `sulcus-local` binary. This ensures sub-50ms context building and allows the agent to function completely offline.
...
```bash
SULCUS_CONFIG=/path/to/sulcus.ini ./target/release/sulcus-local stdio
```
...
```bash
SULCUS_CONFIG=/path/to/sulcus.ini ./target/release/sulcus-local sync-now
```
...
```bash
*/5 * * * * SULCUS_CONFIG=/home/user/sulcus.ini /usr/local/bin/sulcus-local sync-now >> /var/log/sulcus-sync.log 2>&1
```
