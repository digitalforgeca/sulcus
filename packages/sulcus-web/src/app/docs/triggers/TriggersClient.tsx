...
const CONTEXT_XML = `<active_triggers>
  ...
  <trigger name="strategy-boost" event="on_boost" action="webhook" fires="12" filter="@icarus" />
  <trigger name="tag-new-episodic" event="on_store" action="tag" fires="54" filter="episodic@icarus" />
</active_triggers>

<recent_trigger_fires>
  <fire event="on_boost"  action="webhook" node="Strategy: growth"  at="2026-04-06T09:56:32Z" />
  <fire event="on_recall" action="notify"  node="Deploy procedure"  at="2026-04-06T09:50:10Z" />
  <fire event="on_recall" action="notify"  node="User preferences"  at="2026-04-06T09:50:09Z" />
  <fire event="on_store"  action="pin"     node="Architecture"      at="2026-04-06T09:50:03Z" />
</recent_trigger_fires>`;
...
# 3. Cold memory alert — notify when procedure starts decaying
...
# 4. Webhook to Slack when a memory is boosted (priority signal)
client.create_trigger(event="on_boost", action="webhook",
    name="boost-webhook", filter_namespace="icarus",
...
const NODE_EXAMPLES = `import { Sulcus } from "@digitalforgestudios/sulcus";
...
// 3. Cold memory alert — notify when procedural memory decays
...
// 4. Webhook to Slack when a memory is boosted (priority signal)
await client.createTrigger("on_boost", "webhook", {
  name: "boost-webhook", filterNamespace: "icarus",
      ...
      <div className="max-w-3xl mx-auto px-6 py-16 font-sans">
        ...
        <div className="mb-10">
          ...
          <p className="text-[#888] text-base leading-relaxed">
            ...
            graph — a memory is stored, recalled, boosted, or decays. No polling. No cron jobs.
            Evaluated server-side by SITU (Store Intelligence Trigger Unit) on every memory event.
            Your memory governs itself.
          </p>
          <p className="text-xs text-[#555] mt-3 tracking-wider uppercase">
            Sulcus v2.1.0 · Memory Triggers Reference · 2026
          </p>
        </div>
        ...
        <section className="mb-14">
          ...
          <div className="prose prose-invert prose-sm max-w-none text-[#ccc] leading-relaxed space-y-4">
            ...
            <p>
              <strong className="text-[#D4AF37]">No competitor offers this.</strong> Triggers are
              evaluated server-side by <strong className="text-white">SITU</strong> (Store Intelligence
              Trigger Unit), fire organically on memory events, and surface inline as{' '}
        ...
        <section className="mb-14">
          <SectionAnchor id="events" icon={<TbBolt size={20} />} title="Trigger Events"
            sub="Four event types span the full memory lifecycle" />
          <div className="border border-[#00F0FF]/10 divide-y divide-[#00F0FF]/5">
            {([
              ['on_store',    'A new memory node is created',
               'Fires immediately after memory_store or record_memory. Auto-tag, auto-pin, or webhook new content the moment it enters the graph. SITU (Store Intelligence Trigger Unit) evaluates this event server-side.',
               ['Auto-pin by type','Tag on creation','Webhook external systems']],
              ['on_recall',   'A memory is returned by search',
               'Fires for each node returned during memory_recall or search_memory. Ideal for spaced repetition — recalled memories stay hot. Works alongside recall-boost from the thermodynamic engine.',
               ['Spaced repetition','Notify on critical recall','Track what surfaces']],
              ['on_boost',    'Memory heat is explicitly increased',
               'Fires when memory_boost or feedback("relevant") increases heat. Chain actions, log escalations, relay to external systems. Distinct from the automatic recall-boost — this covers manual and trigger-driven heat increases.',
               ['Chain boost actions','Log escalations','Notify on priority shift']],
              ['on_decay',    'Memory heat drops during the decay tick',
               'Fires per-node during the thermodynamic background worker tick (every 300s). Catch important memories before they cool — re-pin, alert, or webhook before expiry.',
          ...
          <div className="space-y-6">
            {([
              ...
              },
              {
                ...
                desc: 'When a procedural memory (how-to, deploy instructions, runbooks) starts to decay, your agent gets an inline notification before the context is lost.',
                event: 'on_decay', action: 'notify', filter: 'filter_memory_type: procedural',
              },
              {
                title: '4. Webhook on priority boost',
                desc: 'When a memory gets its heat boosted — signaling it just became important — fire a signed webhook to Slack. Your team is notified of escalations automatically.',
                event: 'on_boost', action: 'webhook', filter: 'filter_namespace: icarus',
              },
              {
