...
const NODE_QUICKSTART = `import { Sulcus } from "@digitalforgestudios/sulcus";
...
const TRIGGERS_NODE = `import { Sulcus } from "@digitalforgestudios/sulcus";
...
# Add your API key to the config (see above)
# Then restart
...
const API_ENDPOINTS = [
  ...
  { method: 'GET', path: '/api/v1/auth/verify', desc: 'Validate API key — returns identity, tier, limits' },
    ...
    <div className="min-h-screen bg-[#050a0f] text-[#ededed] font-mono selection:bg-[#00F0FF] selection:text-[#050a0f] relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto px-6 relative z-10">
        ...
        <section className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <CodeBlock code={`pip install sulcus`} lang="bash" />
              <p className="text-xs text-[#555] mt-2">Python 3.9+ · Zero dependencies</p>
            </div>
            <div>
              <h3 className="text-sm font-bold tracking-widest uppercase text-[#D4AF37] mb-3">Node.js</h3>
              <CodeBlock code={`npm install @digitalforgestudios/sulcus`} lang="bash" />
        ...
        <section id="openclaw" className="mb-20">
          ...
          <p className="text-[#888] mb-6">
            ...
            Replace file-based memory with reactive, thermodynamic memory — your agents get heat-based decay, 
        ...
        <section className="mb-20">
          ...
          <div className="border border-[#00F0FF]/10 divide-y divide-[#00F0FF]/5">
            {[
              { name: 'Node.js SDK', install: 'npm install @digitalforgestudios/sulcus', desc: 'Core TypeScript client with full API coverage', status: '✅' },
              { name: 'Python SDK', install: 'pip install sulcus', desc: 'Core Python client for any framework', status: '✅' },
              { name: 'OpenClaw', install: 'openclaw plugin install @digitalforgestudios/openclaw-sulcus', desc: 'Full memory backend: auto-recall, auto-capture, triggers', status: '✅' },
              { name: 'REST API', install: 'curl -H "Authorization: Bearer $KEY" api.sulcus.ca/...', desc: 'Direct HTTP — works with any language', status: '✅' },
              { name: 'LangChain', install: 'Coming soon', desc: 'SulcusMemory ChatMessageHistory + retriever', status: '🔜' },
              { name: 'LlamaIndex', install: 'Coming soon', desc: 'Memory store + query engine integration', status: '🔜' },
              { name: 'Vercel AI SDK', install: 'Coming soon', desc: 'LanguageModelV3Middleware for automatic memory', status: '🔜' },
              { name: 'CrewAI', install: 'Coming soon', desc: 'Shared reactive, thermodynamic memory for multi-agent crews', status: '🔜' },
              { name: 'CLI', install: 'Coming soon', desc: 'Terminal interface: search, store, list, pin, forget', status: '🔜' },
            ].map((i, idx) => (
              <div key={idx} className="flex items-center gap-4 px-4 py-3 hover:bg-[#00F0FF]/5 transition-colors">
                <span className="text-xs w-6">{i.status}</span>
                <span className="text-sm text-white font-medium w-28">{i.name}</span>
          ...
          <p className="text-xs text-[#555] mt-4">
            Available packages on{' '}
            <a href="https://www.npmjs.com/package/@digitalforgestudios/sulcus" className="text-[#00F0FF] hover:underline">
              npm
            </a>
            {' '}and{' '}
        ...
        <section className="border-t border-[#00F0FF]/10 pt-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
            <a href="https://github.com/digitalforgeca/sulcus" className="border border-[#00F0FF]/10 p-4 hover:border-[#00F0FF]/30 transition-colors block">
              ...
              <div className="text-[#666]">Source, SDKs, and examples</div>
            </a>
            ...
            <a href="https://www.npmjs.com/package/@digitalforgestudios/sulcus" className="border border-[#00F0FF]/10 p-4 hover:border-[#00F0FF]/30 transition-colors block">
              <div className="font-bold text-white mb-1">npm</div>
              <div className="text-[#666]">@digitalforgestudios/sulcus</div>
            </a>
            ...
            <Link href="/docs/training" className="border border-[#00F0FF]/10 p-4 hover:border-[#00F0FF]/30 transition-colors block">
              <div className="font-bold text-white mb-1">Training Signals</div>
              <div className="text-[#666]">Memory lifecycle actions that train SIVU, SICU, SILU, and SITU</div>
            </Link>
            ...
            <Link href="/docs/sdks" className="border border-[#00F0FF]/10 p-4 hover:border-[#00F0FF]/30 transition-colors block">
              <div className="font-bold text-white mb-1">SDKs &amp; Integrations</div>
              <div className="text-[#666]">Node.js, Python, OpenClaw, REST API</div>
            </Link>
            <Link href="/docs/knowledge-graph" className="border border-[#00F0FF]/10 p-4 hover:border-[#00F0FF]/30 transition-colors block">
              <div className="font-bold text-white mb-1">Knowledge Graph (AGE)</div>
              <div className="text-[#666]">Apache AGE temporal graph — Cypher queries, SILU entity extraction, temporal traversal</div>
            </Link>
            <Link href="/docs/changelog" className="border border-[#D4AF37]/20 p-4 hover:border-[#D4AF37]/40 transition-colors block">
              <div className="font-bold text-[#D4AF37] mb-1">Changelog</div>
              <div className="text-[#666]">What&apos;s new — release notes and version history</div>
            </Link>
            <Link href="/docs/curator" className="border border-[#00F0FF]/10 p-4 hover:border-[#00F0FF]/30 transition-colors block">
              <div className="font-bold text-white mb-1">Curator System</div>
              <div className="text-[#666]">Background curation cycle — reclassifies, consolidates, summarizes, and re-vectorizes memories</div>
            </Link>
          </div>
        </section>
        </div>
      ...
      <footer className="py-12 border-t border-[#D4AF37]/20 text-center relative z-10">
        <p className="text-[10px] text-[#2a4a5a] tracking-[0.3em] font-medium uppercase hover:text-[#00F0FF]/50 transition-colors cursor-default">
          © 2026 Digital Forge Studios Inc. Tempered by thermodynamics.
        </p>
      </footer>
    </div>
  );
