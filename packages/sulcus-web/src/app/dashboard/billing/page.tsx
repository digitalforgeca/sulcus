...
import { TIERS, PAID_TIER_KEYS, getTier, normalizeTierKey } from "@/lib/pricing";
...
// Sulcus product tiers — filter from shared pricing config
const SULCUS_TIERS = PAID_TIER_KEYS;
...
function BillingContent() {
  ...
  useEffect(() => {
    ...
    async function loadOrg() {
      try {
        ...
        setCurrentTier(normalizeTierKey(raw));
      } catch (err) {
  ...
  // Detect unlimited: server returns 0 or very large numbers (i64::MAX) for unlimited quotas
  ...
  const limits = {
    sync_requests: isOpsUnlimited ? Infinity : rawOps,
    nodes: isNodesUnlimited ? Infinity : rawNodes,
  };
  const syncPct = usage && !isOpsUnlimited
    ? Math.min((usage.sync_requests / limits.sync_requests) * 100, 100)
    : 0;
  const nodesPct = usage && !isNodesUnlimited
      ...
      <div className="bg-[#0a1520] p-8 rounded-lg border border-[#D4AF37]/30 shadow-[0_0_15px_rgba(212,175,55,0.05)] relative mb-10">
        ...
        <h2 className="text-xl font-bold mb-2 text-white uppercase tracking-widest">
          ...
          {loadingTier || currentTier === null ? (
            ...
            return t ? `${t.emoji} ${t.name}${t.price === 0 ? ' (Free)' : ''}`.trim() : currentTier;
          })()}
        </h2>
        <p className="text-[#888] mb-6 text-sm">
          {currentTier === null || loadingTier
            ? ""
            : getTier(currentTier)?.description ?? ""}
        </p>
            ...
            <div className="bg-[#111820] p-4 border border-[#D4AF37]/20">
              <div className="flex justify-between mb-2">
                ...
                <span className="text-xs font-bold text-[#D4AF37]">
                  ...
                  {isOpsUnlimited ? "∞" : limits.sync_requests.toLocaleString()}
                </span>
              </div>
            ...
            <div className="bg-[#111820] p-4 border border-[#D4AF37]/20">
              <div className="flex justify-between mb-2">
                ...
                <span className="text-xs font-bold text-[#D4AF37]">
                  ...
                  {isNodesUnlimited ? "∞" : limits.nodes.toLocaleString()}
                </span>
              </div>
              <div className="w-full bg-black h-1">
                <div
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-10">
          ...
          {sortedProducts.map((product) => {
            ...
            const interval = price?.recurring?.interval
              ...
              : "";
            const tierConfig = product.metadata?.tier ? getTier(product.metadata.tier) : undefined;
            const isHighlighted = tierConfig?.highlighted ?? false;
            ...
            return (
              <div
                key={product.id}
                className={`bg-[#0a1520] p-6 border relative flex flex-col ${isActive ? "border-[#00F0FF]/50" : isHighlighted ? "border-[#D4AF37]/40" : "border-[#333]"}`}
              >
                <div
                  className={`absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent ${isActive ? "via-[#00F0FF]" : isHighlighted ? "via-[#D4AF37]" : "via-[#555]"} to-transparent`}
                ></div>
                {isActive ? (
                  <div className="text-xs uppercase tracking-widest text-[#00F0FF] mb-2">
                    Current Plan
                  ...
                  <div className="text-xs uppercase tracking-widest text-[#D4AF37] mb-2">
                    {tierConfig?.badge ?? "Recommended"}
                  </div>
                ) : (
                  <div className="text-xs uppercase tracking-widest text-[#555] mb-2">
                    {tierConfig?.dashboardLabel ?? ""}
                  </div>
                )}
                <h3
                  className={`text-lg font-bold tracking-widest uppercase mb-1 ${isHighlighted ? "text-[#D4AF37]" : "text-white"}`}
                >
                ...
                <ul className="text-[#888] text-sm space-y-2 flex-1 mb-4">
                  {featureItems.map((feat, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <span
                        className={isHighlighted ? "text-[#D4AF37]" : "text-[#555]"}
                      >
                        ✓
                      </span>
                      {feat}
