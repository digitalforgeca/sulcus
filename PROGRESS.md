...
- [x] Initialize Cargo workspace and members (`sulcus-core`, `sulcus`, `sulcus-server`).
...
- [x] Add minimal placeholder crates for `sulcus` and `sulcus-server`.
- [x] Implement `StorageBackend` adapter for PostgreSQL-compatible local storage in `sulcus`.
