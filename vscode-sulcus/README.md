...
Minimal VS Code extension to interact with a local `sulcus-local` sidecar.

Commands

- `Sulcus: Summarize Selection (local)` — call `sulcus-local summarize` on the active selection or document.
- `Sulcus: Add Memory (local)` — record selected text as a Sulcus memory (upserts node).
- `Sulcus: Show Active Index` — display `sulcus-local show-active` output.
- `Sulcus: Send Last Summary to SUMR` — if SUMR extension is installed, send last generated summary to SUMR.

Configuration

- `sulcus.binPath` — path to `sulcus-local` binary (default: `sulcus-local`).
- `sulcus.maxSummaryChars` — maximum chars requested from `sulcus-local` summarize (default: 500).

Usage

Extension ID: `your-publisher.sulcus-vscode` — replace `your-publisher` with your publisher ID when publishing.

1. Ensure `sulcus-local` is in your PATH (or set `sulcus.binPath`).
...
- This is a thin helper: `sulcus-local` still does the work. The extension just shells out and shows results.
