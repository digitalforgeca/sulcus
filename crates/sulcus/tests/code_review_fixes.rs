...
use sulcus::{LocalStorage, McpHandler, MockEmbeddingProvider};
use uuid::Uuid;
...
async fn fix5a_tick_hub_node_completes_under_five_seconds() -> anyhow::Result<()> {
    ...
    sulcus::tick(&storage, 0.85, 0.01, 20).await?;
...
async fn fix5b_build_context_respects_token_budget() -> anyhow::Result<()> {
    ...
    let actual_tokens = sulcus::count_tokens(content_text) as u64;
