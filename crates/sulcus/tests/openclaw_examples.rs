...
fn node_example_runs() -> anyhow::Result<()> {
    ...
    let sulcus_bin = std::env::var("CARGO_BIN_EXE_sulcus").ok().or_else(|| {
        // fallback: workspace target/debug/sulcus
        ...
        None
    }).expect("sulcus binary not found; build with `cargo build -p sulcus` before running this test");
