...
use sulcus_core::sync::{MemoryOp, SyncEngine, SyncPullResult, SyncPushResult};
use sulcus_core::StorageBackend;
use sulcus::sync::LocalSyncClient;
use uuid::Uuid;
...
async fn test_differential_sync_filters_by_heat() -> anyhow::Result<()> {
    ...
    storage
        .record_memory_op(
            "ADD",
            &serde_json::json!({"id": hot_id.to_string(), "label": "Hot"}),
        )
        .await?;
    storage
        .upsert_node(sulcus_core::graph::Node {
    ...
    storage
        .record_memory_op(
            "ADD",
            &serde_json::json!({"id": cold_id.to_string(), "label": "Cold"}),
        )
        .await?;
