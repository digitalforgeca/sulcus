...
fn find_sulcus_bin() -> String {
    ...
    if let Some(ws) = workspace {
        let candidate = ws.join("target/debug/sulcus");
        if candidate.exists() {
            return candidate.to_string_lossy().to_string();
        }
    }
    panic!("sulcus binary not found; run `cargo build -p sulcus` first");
}
...
/// Ensure embedded PG is running (via sulcus's `initialize`) and return the base URL.
