...
//!
//! On startup, sulcus reads the manifest, checks each expected library
...
pub struct LibEntry {
    /// Human-readable name (e.g. "sulcus-vectors").
    pub name: &'static str,
...
fn build_manifest() -> Vec<LibEntry> {
    vec![
        LibEntry {
            name: "sulcus-vectors",
            filename: dylib_name("sulcus_vectors"),
        ...
        LibEntry {
            ...
            // Optional: sulcus still has sqlx + pg-embed compiled in (phase 2 will extract).
...
pub fn check_or_die() {
    ...
    if !report.all_required_found {
        eprintln!("{report}");
        eprintln!("FATAL: Required libraries missing. Cannot start sulcus.");
