...
const DEFAULT_PG_PORT: u16 = 4201;
...
fn ensure_local_dirs(base: &PathBuf) -> anyhow::Result<PathBuf> {
    ...
    // Clean up stale postmaster.pid — if the PID in the file is dead, remove the lock
    // so pg-embed can start. Without this, a crashed sulcus leaves an orphaned
    // lock file that permanently blocks embedded PG from restarting.
    let pid_file = postgres_dir.join("postmaster.pid");
    if pid_file.exists() {
            ...
            if let Some(pid_str) = contents.lines().next() {
                if let Ok(pid) = pid_str.trim().parse::<u32>() {
                    #[cfg(unix)]
                    {
                        ...
                        if !alive {
                            tracing::warn!(
                                "removing stale postmaster.pid (PID {} is not running)",
                                pid
                            );
...
/// Start the embedded PostgreSQL 17 instance via pg-embed.
///
/// This is the sole embedded database strategy for sulcus.
/// PGLite (WASM-based) was removed — it crashes on macOS arm64 and adds
/// a Node.js dependency. pg-embed downloads a real PG binary on first run
/// and manages it as a child process. Data is persisted to ~/.sulcus/local/postgres/.
async fn start_embedded_pg() -> anyhow::Result<String> {
    ...
    // If an existing PG is already running on our port, reuse it
    if probe_connect(&db_url).await.is_ok() {
        tracing::info!("embedded PG already running on port {}", port);
        return Ok(db_url);
    }

    tracing::info!("starting embedded PostgreSQL 17 (pg-embed) on port {}...", port);
    ...
    if guard.is_none() {
        ...
        let mut pg = PgEmbed::new(pg_settings, fetch_settings)
            .await
            .map_err(|e| anyhow::anyhow!("pg-embed init failed: {e}"))?;
        pg.setup().await.map_err(|e| anyhow::anyhow!("pg-embed setup failed: {e}"))?;
        pg.start_db().await.map_err(|e| anyhow::anyhow!("pg-embed start failed: {e}"))?;
        *guard = Some(pg);
    }
...
async fn run_migrations(db_url: &str) -> anyhow::Result<()> {
    ...
    for migration_sql in [
        ...
        include_str!("../migrations/0013_siu_config.sql"),
        include_str!("../migrations/0014_training_signals.sql"),
    ] {
...
fn create_embedder() -> std::sync::Arc<dyn crate::embeddings::EmbeddingProvider> {
    ...
    match crate::embeddings::FastEmbedProvider::try_new() {
        Ok(embedder) => {
            tracing::info!("embedding provider ready (via sulcus-vectors dylib)");
...
pub async fn serve(
    ...
    let panel_routes = Router::new()
        ...
        .route("/api/v1/agent/nodes", get(crate::local_api::list_memories).post(crate::local_api::create_node))
        .route(
        ...
        .route(
            "/api/v1/settings/siu",
            get(crate::local_api::get_siu_config).patch(crate::local_api::patch_siu_config),
        )
