    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "time_from": { "type": "string", "description": "ISO 8601 datetime — only return memories created after this time" },
                "time_to": { "type": "string", "description": "ISO 8601 datetime — only return memories created before this time" }
            }
        })
    }
    async fn call(&self, handler: &McpHandler, args: Value) -> anyhow::Result<Value> {
        ...
        let time_from = args.get("time_from").and_then(|x| x.as_str());
        let time_to = args.get("time_to").and_then(|x| x.as_str());
        ...
        // Temporal filtering: retain only nodes within the requested time range
        if time_from.is_some() || time_to.is_some() {
            let time_ids: Vec<String> = match (time_from, time_to) {
                (Some(from), Some(to)) => sqlx::query(
                    "SELECT id FROM nodes WHERE created_at >= $1::timestamptz AND created_at <= $2::timestamptz",
                )
                .bind(from)
                .bind(to)
                .fetch_all(handler.storage().pool())
                ...
                (Some(from), None) => sqlx::query(
                    "SELECT id FROM nodes WHERE created_at >= $1::timestamptz",
                )
                .bind(from)
                .fetch_all(handler.storage().pool())
                .await
            ...
            let id_set: std::collections::HashSet<String> = time_ids.into_iter().collect();
            scores.retain(|id, _| id_set.contains(id));
        }
...
#[async_trait]
impl McpTool for EvaluateOutput {
    fn name(&self) -> &str {
        "evaluate_output"
    }
    fn description(&self) -> &str {
        "Evaluate LLM output against agent memory for semantic alignment (SILU recursive supervisor)"
    }
    fn input_schema(&self) -> Value {
        json!({
            "type": "object",
            ...
            "properties": {
                "output": {
                    "type": "string",
                    "description": "The LLM output text to evaluate for semantic alignment"
                },
                "prompt_summary": {
                    "type": "string",
                    "description": "Optional summary of the prompt that produced this output"
                }
            }
        })
    }
    async fn call(&self, handler: &McpHandler, args: Value) -> anyhow::Result<Value> {
        let output = args.get("output").and_then(|x| x.as_str()).unwrap_or("");
        let prompt_summary = args.get("prompt_summary").and_then(|x| x.as_str());
        ...
        match (server_url, api_key) {
            (Some(url), Some(key)) => {
                ...
                if let Some(ps) = prompt_summary {
                    context.insert("prompt_summary".to_string(), json!(ps));
                }
                let body = json!({
                    "output": output,
                    "context": context
                ...
                let resp = reqwest::Client::new()
                    .post(format!("{}/api/v1/agent/evaluate-output", url))
            ...
            _ => Ok(json!({
                "error": "Output evaluation requires cloud backend (sulcus.ca). Configure SULCUS_SERVER_URL and SULCUS_API_KEY."
            })),
        }
    }
}
