...
async fn main() -> anyhow::Result<()> {
    ...
    sulcus::manifest::check_or_die();

    if args.len() < 2 {
        eprintln!(
            "sulcus {} — reactive, thermodynamic memory sidecar",
            env!("CARGO_PKG_VERSION")
        );
        eprintln!();
        eprintln!("Usage: sulcus [--config <path>] <command> [args]");
    ...
    let config = sulcus::config::Config::load();
    ...
    if let Some(ref db_url) = db {
        if db_url.starts_with("sqlite:") {
            anyhow::bail!(
                "SQLite DSNs are not supported in sulcus. Unset SULCUS_DATABASE_URL to use embedded PostgreSQL (pg-embed), or set it to a reachable PostgreSQL-compatible URL."
            );
        }
    }
    ...
    match cmd {
        "serve" => {
            sulcus::serve(db.as_deref(), interval_ms, active_limit).await?;
            Ok(())
        }
        "stdio" => {
            sulcus::serve_stdio(db.as_deref(), interval_ms, active_limit).await?;
            Ok(())
        }
        "init" => {
            let url = sulcus::initialize(db.as_deref()).await?;
        ...
        "reinit" => {
            ...
            let url = if force_external && db.is_some() {
                ...
                sulcus::initialize(db.as_deref()).await?
            } else {
                sulcus::reinitialize_local().await?
            };
        ...
        "demo" => {
            ...
            let db_url = sulcus::initialize(db.as_deref()).await?;
            let storage = sulcus::LocalStorage::new(&db_url).await?;
        ...
        "add-memory" => {
            if args.len() < 3 {
                eprintln!("Usage: sulcus add-memory <summary> [heat]");
                std::process::exit(1);
            }
            let summary = &args[2];
            let heat = args
        ...
        "summarize" => {
            ...
            let db_url = sulcus::initialize(db.as_deref()).await?;
            let storage = sulcus::LocalStorage::new(&db_url).await?;
            let embedder: Arc<dyn sulcus::EmbeddingProvider> =
                Arc::new(sulcus::FastEmbedProvider::try_new()?);
            let handler = sulcus::McpHandler::new(storage, embedder, active_limit);
...
async fn maybe_shutdown_embedded(db_url: Option<&str>) {
    if db_url.is_none() {
        let _ = sulcus::shutdown_embedded_postgres().await;
    }
}
