//! sulcus-vectors — shared library for embedding operations.
//!
...
//! The main `sulcus` binary loads this via `dlopen` through the progressive
...
/// The caller (sulcus) owns the pointer and must free it with
/// `sulcus_vectors_destroy`.
///
/// # Safety
/// Returns a raw pointer. Caller must eventually call `sulcus_vectors_destroy`.
#[no_mangle]
pub unsafe extern "C" fn sulcus_vectors_create() -> *mut VectorsHandle {
    ...
    let result = std::panic::catch_unwind(|| VectorsHandle::new());
    match result {
        Ok(Ok(handle)) => Box::into_raw(Box::new(handle)),
        Ok(Err(e)) => {
            eprintln!("[sulcus-vectors] failed to create embedding provider: {e}");
            std::ptr::null_mut()
        }
        Err(_panic) => {
            eprintln!("[sulcus-vectors] embedding provider panicked (ORT/ONNX unavailable) — falling back to mock embeddings");
            std::ptr::null_mut()
        }
    }
}

/// Destroy an embedding handle returned by `sulcus_vectors_create`.
///
/// # Safety
/// Must be a valid pointer returned by `sulcus_vectors_create` and not yet freed.
#[no_mangle]
pub unsafe extern "C" fn sulcus_vectors_destroy(handle: *mut VectorsHandle) {
    if !handle.is_null() {
...
/// with `sulcus_vectors_free_string`.
///
/// # Safety
/// `handle` must be a valid pointer. `text` must be a valid null-terminated UTF-8 string.
#[no_mangle]
pub unsafe extern "C" fn sulcus_vectors_text(
    handle: *const VectorsHandle,
...
#[no_mangle]
pub unsafe extern "C" fn sulcus_vectors_batch(
    handle: *const VectorsHandle,
...
#[no_mangle]
pub unsafe extern "C" fn sulcus_vectors_count_tokens(
    text: *const std::ffi::c_char,
) -> i64 {
    if text.is_null() {
        return -1;
...
/// Free a string returned by any `sulcus_vectors_*` function.
///
/// # Safety
/// Must be a valid pointer returned by this library and not yet freed.
#[no_mangle]
pub unsafe extern "C" fn sulcus_vectors_free_string(s: *mut std::ffi::c_char) {
    if !s.is_null() {
...
#[no_mangle]
pub extern "C" fn sulcus_vectors_version() -> *const std::ffi::c_char {
