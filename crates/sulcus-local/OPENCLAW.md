...
// add memory
cp.stdin.write(JSON.stringify({ id: '1', method: 'add_memory', params: { content: 'User said: hello' } }) + '\n');

// query active index
cp.stdin.write(JSON.stringify({ id: '2', method: 'resource', params: { resource: 'memory://active_index', limit: 10 } }) + '\n');
```
...
Tests
-----
`crates/sulcus-local/tests/openclaw_integration.rs` demonstrates end-to-end MCP-over-stdio interaction used by OpenClaw.
...
If you want, I can add a small OpenClaw example plugin (Node/Python) that wraps this pattern — tell me which language you prefer.