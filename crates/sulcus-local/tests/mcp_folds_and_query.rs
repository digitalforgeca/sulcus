...
async fn record_and_query_memory_via_mcp_tooling() -> anyhow::Result<()> {
    ...
    let resp_s = handler.handle_request(&req.to_string()).await?;
    let resp: Value = serde_json::from_str(&resp_s)?;
    let content_text = resp.get("result").and_then(|r| r.get("content")).and_then(|c| c[0].get("text")).and_then(|t| t.as_str())
        .expect(&format!("no text in search_memory response: {}", resp_s));
    let inner: Value = serde_json::from_str(content_text)?;
    let results = inner.get("results").and_then(|r| r.as_array())
        .expect("no results array in response");
    // node_id must appear somewhere in the result set (FTS on pointer_summary)
    assert!(
        results.iter().any(|r| r.get("id").and_then(|i| i.as_str()) == Some(node_id)),
        "stored node {} not found in search results: {:?}", node_id, results
    );
