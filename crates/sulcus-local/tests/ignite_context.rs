...
impl sulcus_local::embeddings::EmbeddingProvider for UniqueEmbeddingProvider {
...
async fn thermodynamics_ignite_context_inserts_heat_and_runs_tick() -> anyhow::Result<()> {
    ...
    assert!((na.current_heat - 0.8).abs() < 1e-4, "A heat should be ~0.8 after ignite+tick decay (got {})", na.current_heat);
