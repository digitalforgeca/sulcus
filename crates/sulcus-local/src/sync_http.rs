...
use sulcus_core::sync::{SyncEngine, MemoryOp};
use rand::Rng;
...
impl HttpSyncEngine {
    pub fn new(base_url: impl Into<String>, api_key: Option<String>) -> Self {
        Self { client: reqwest::Client::new(), base_url: base_url.into(), api_key, max_retries: 3, backoff_base_ms: 200 }
    }
    ...
    async fn send_with_retry(&self, mut req: reqwest::RequestBuilder) -> anyhow::Result<reqwest::Response> {
...
impl SyncEngine for HttpSyncEngine {
    async fn push(&self, ops: Vec<MemoryOp>) -> anyhow::Result<sulcus_core::sync::SyncPushResult> {
        ...
        let new_cursor = j.get("new_cursor").and_then(|v| v.as_str()).map(|s| s.to_string());
        let new_cursor_seq = j.get("new_cursor_seq").and_then(|v| v.as_i64());
        Ok(sulcus_core::sync::SyncPushResult { new_cursor, new_cursor_seq })
    }

    async fn pull(&self, since: Option<chrono::DateTime<chrono::Utc>>) -> anyhow::Result<sulcus_core::sync::SyncPullResult> {
        ...
        let new_cursor = j.get("new_cursor").and_then(|v| v.as_str()).map(|s| s.to_string());
        let new_cursor_seq = j.get("new_cursor_seq").and_then(|v| v.as_i64());
        Ok(sulcus_core::sync::SyncPullResult { ops, new_cursor, new_cursor_seq })
    }
}
                ...
                Ok::<_, hyper::Error>(service_fn(move |req: Request<Body>| {
                    ...
                    async move {
                        if req.method() == Method::POST && req.uri().path() == "/api/v1/agent/sync" {
                            ...
                            Ok::<_, hyper::Error>(Response::builder().status(StatusCode::NOT_FOUND).body(Body::empty()).unwrap())
                        }
                    }
                }))
            }
