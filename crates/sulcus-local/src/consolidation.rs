...
pub async fn consolidate_hot_clusters(storage: &LocalStorage) -> anyhow::Result<usize> {
    ...
    for ns_row in ns_rows.iter() {
        ...
        let mut member_labels: Vec<String> = Vec::with_capacity(node_rows.len());
        ...
        //    Uses ON CONFLICT to update edge_weight if already present.
    ...
    //    Only touches nodes not already covered by the synthesis pass above.
...
/// and cluster members. Uses UUIDv5 (SHA-1, DNS namespace) so that re-running
/// consolidation on the same cluster upserts rather than duplicates. Member
/// order is normalised (sorted) before hashing.
fn synthesise_node_id(namespace: &str, member_ids: &[String]) -> String {
    ...
    // Use the DNS namespace UUID as the UUIDv5 namespace (stable well-known value).
...
mod tests {
    ...
    fn synthesis_node_id_is_deterministic() {
        ...
        // Order of members should not matter (they're sorted internally).
    ...
    fn synthesis_node_id_is_valid_uuid() {
        ...
        assert!(Uuid::parse_str(&id).is_ok(), "must be a parseable UUID: {id}");
    }
    ...
    fn extractive_summary_truncates() {
        ...
        // Should contain first 3, not the fourth
    ...
    #[test]
    fn extractive_summary_handles_empty() {
        let summary = extractive_cluster_summary("");
        assert!(summary.is_empty() || summary.len() < 10);
    }
    ...
    fn extractive_summary_strips_label_prefix() {
        ...
        let summary = extractive_cluster_summary(corpus);
        assert!(!summary.contains("MyLabel"), "label prefix should be stripped");
