...
-- so the DB can enforce idempotency at the constraint/index level. Tenant-scoped.
...
-- unique fingerprint to make the server WAL idempotent for duplicate client ops (scoped by tenant)
...
CREATE INDEX IF NOT EXISTS idx_golden_index_current_heat_updated_at ON golden_index (tenant_id, current_heat DESC, updated_at DESC);
