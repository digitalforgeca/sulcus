...
pub async fn get_org(
    ...
    // Get org info from api_keys (if exists)
    ...
    .await
    {
        Ok(Some(r)) => Some(r),
        Ok(None) => {
            // No api_keys row, but the tenant IS authenticated (middleware passed).
            // Fall through — we'll build the response from TenantContext instead.
            tracing::info!(tenant_id = %tenant_id, "get_org: no api_keys row, using TenantContext");
            None
        }
        Err(e) => {
    ...
    // Try to get org name from tenant_kc_orgs if api_keys doesn't have one
    let kc_org_name: Option<String> = if row.as_ref().and_then(|r| r.0.as_ref()).is_none() {
    ...
    // Build org info from api_keys row (if exists) or TenantContext (fallback)
    let (org_name, plan_tier, max_seats, seats_used, row_features) = match row {
        Some(r) => (r.0, r.1, r.2, r.3, r.4.unwrap_or_default()),
        None => (
            kc_org_name,
            tenant_ctx.plan_tier.clone(),
            None,
            1,
            String::new(),
        ),
