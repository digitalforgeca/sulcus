...
use sulcus::{LocalStorage, McpHandler};
...
pub struct McpManager {
    ...
    pub embedder: Arc<dyn sulcus::embeddings::EmbeddingProvider>,
}
...
impl McpManager {
    pub fn new() -> Self {
        // FastEmbedProvider now loads libsulcus_vectors.dylib via FFI.
        // If the dylib is missing, try_new() fails gracefully and we fall back to mock.
        let embedder: Arc<dyn sulcus::embeddings::EmbeddingProvider> =
            if sulcus::FastEmbedProvider::is_available() {
                match sulcus::FastEmbedProvider::try_new() {
                    Ok(p) => {
                        tracing::info!("MCP embedder initialized (via sulcus-vectors dylib)");
                        Arc::new(p) as Arc<dyn sulcus::embeddings::EmbeddingProvider>
                    }
                    Err(e) => {
                        tracing::warn!(error = %e, "MCP embedder fallback to mock (dylib init failed)");
                        Arc::new(sulcus::MockEmbeddingProvider::new())
                    }
                }
            } else {
