...
pub async fn verify_and_provision_jit(
    ...
    // 2. Check oidc_tenant_links table (persistent, survives key revocation)
    // 3. Fall back to api_keys.keycloak_user_id (legacy)
    // 4. JIT-provision if enabled, otherwise reject
    let tenant_id = if let Some(org) = &claims.org_id {
        org.clone()
    } else {
        // 2. Check dedicated OIDC links table first
        let oidc_link = sqlx::query(
            "SELECT tenant_id FROM oidc_tenant_links WHERE keycloak_user_id = $1",
        )
        .bind(&claims.sub)
        .fetch_optional(pool)
        .await?;

        if let Some(row) = oidc_link {
            let tid: String = row.get("tenant_id");
            tracing::info!(tenant_id = %tid, "OIDC: resolved tenant from oidc_tenant_links");
            tid
        } else {
            // 3. Legacy fallback: check api_keys.keycloak_user_id
            ...
            if let Some(row) = existing {
                ...
                tracing::info!(tenant_id = %tid, existing_tier = %existing_tier, "OIDC JIT: found tenant via api_keys (legacy), migrating to oidc_tenant_links");

                // Auto-migrate to the links table so future lookups are key-independent
                let _ = sqlx::query(
                ...
                // Also create the link for future resilience
                let _ = sqlx::query(
                    "INSERT INTO oidc_tenant_links (keycloak_user_id, tenant_id) VALUES ($1, $2) ON CONFLICT (keycloak_user_id) DO NOTHING"
                )
                .bind(&claims.sub)
