...
pub async fn handle_text_search(
    ...
    match rows {
        Ok(mut rows) => {
            ...
            // ── Cross-encoder reranker pass (Task 92) ──────────────────────
            // When use_reranker=true, pass the top `reranker_top_k` candidates
            // to the local cross-encoder sidecar for a joint (query, doc) score.
            // The reranker scores are used to re-order candidates before slicing
            // to `limit`. Falls back to fused score if sidecar is unavailable.
            let use_reranker = thermo_config.recall.use_reranker;
            let reranker_top_k = thermo_config.recall.reranker_top_k as usize;

            let scored_results = if use_reranker && !req.query.is_empty() {
                if let Some(ref reranker_client) = state.reranker {
                    // Prepare candidates: take top min(reranker_top_k, all) by fused score
                    // Use a separate scope to end the borrow before we might move scored_results.
                    let n_candidates = scored_results.len().min(reranker_top_k);
                    let candidates: Vec<(String, String)> = {
                        scored_results[..n_candidates]
                            .iter()
                            .filter_map(|(_, v)| {
                                let id = v.get("id").and_then(|x| x.as_str()).map(String::from)?;
                                let text = v.get("pointer_summary").and_then(|x| x.as_str()).map(String::from)?;
                                Some((id, text))
                            })
                            .collect()
                    }; // borrow of scored_results ends here

                    let reranked_opt = reranker_client.rerank(&req.query, candidates).await;

                    if let Some(reranked_scores) = reranked_opt {
                        // Build ID → cross-encoder score lookup
                        let score_map: std::collections::HashMap<String, f32> =
                            reranked_scores.into_iter().map(|s| (s.id, s.score)).collect();

                        // Compute score range for normalization
                        let reranked_max = score_map.values().copied().fold(f32::NEG_INFINITY, f32::max);
                        let reranked_min = score_map.values().copied().fold(f32::INFINITY, f32::min);
                        let fused_scale = if reranked_max > reranked_min {
                            (reranked_max - reranked_min) / (reranked_max.abs().max(1.0))
                        } else {
                            1.0
                        };

                        let mut re_scored: Vec<(f32, serde_json::Value)> = scored_results
                            .into_iter()
                            .enumerate()
                            .map(|(i, (fused, mut v))| {
                                let id = v.get("id").and_then(|x| x.as_str()).unwrap_or("").to_string();
                                if let Some(&ce_score) = score_map.get(&id) {
                                    // Blend: 80% cross-encoder + 20% fused (normalised to same scale)
                                    let normalized_fused = fused * fused_scale;
                                    let blended = 0.80 * ce_score + 0.20 * normalized_fused;
                                    if req.explain {
                                        if let Some(obj) = v.as_object_mut() {
                                            if let Some(exp) = obj.get_mut("explain") {
                                                if let Some(exp_obj) = exp.as_object_mut() {
                                                    exp_obj.insert("cross_encoder_score".to_string(), serde_json::json!(ce_score));
                                                    exp_obj.insert("reranker_blend".to_string(), serde_json::json!(blended));
                                                    exp_obj.insert("reranked".to_string(), serde_json::json!(true));
                                                }
                                            }
                                        }
                                    }
                                    v["score"] = serde_json::json!(blended);
                                    (blended, v)
                                } else {
                                    // Beyond reranker_top_k — shift below reranked floor
                                    let penalty = reranked_min - (i as f32 * 0.001);
                                    (penalty, v)
                                }
                            })
                            .collect();

                        re_scored.sort_by(|a, b| b.0.total_cmp(&a.0));
                        tracing::debug!(
                        ...
                        // Reranker unavailable — fall back to fused score
                        tracing::debug!("Reranker unavailable, using fused score");
                        scored_results
                    }
                } else {
                    scored_results
