...
async fn test_tenant_isolation() {
    ...
    let req_a = Request::builder()
        ...
        .body(Body::from(json!({ "ops": [op_a], "last_cursor": null }).to_string()))
        .unwrap();
    ...
    let req_b = Request::builder()
        ...
        .header("Content-Type", "application/json")
        .body(Body::from(json!({ "ops": [], "last_cursor": null }).to_string()))
        .unwrap();

    let res_b = app.clone().oneshot(req_b).await.unwrap();
    assert_eq!(res_b.status(), StatusCode::OK);
    let body_bytes = axum::body::to_bytes(res_b.into_body(), usize::MAX).await.unwrap();
    ...
    let body_bytes_a2 = axum::body::to_bytes(res_a2.into_body(), usize::MAX).await.unwrap();
