...
## Repository
- Local: `/Users/devuser2/dev/sulcus`
- Remote: `git@github.com:devuser/sulcus.git`
...
```bash
cd /Users/devuser2/dev/sulcus && git log --oneline -10
```

## Tech Stack
- **Core Library (`sulcus-core`):** Rust — Node/Edge models, HLC-CRDTs, ACT-R thermodynamics
- **Local Sidecar (`sulcus`):** Rust — PGlite/Postgres adapter, Stdio MCP Server, FastEmbed ONNX
...
- [ ] Publish `sulcus-core` and `sulcus` crates to `crates.io`
...
cd /Users/devuser2/dev/sulcus/dashboard && npm run build 2>/dev/null || pnpm build
```
...
3. Append to `/Users/devuser2/.openclaw/workspace/output/status.txt`:
