...
This is the critical "Open Core" pivot. You are right: if you put the `sulcus-server` code in the same MIT-licensed GitHub repository as `sulcus`, you have destroyed your business model before writing a line of code.
...
**The Result:** A user can compile `sulcus` all day long. It physically lacks the code required to run a sync server. To get that feature, they must download your proprietary binary or pay for the SaaS.
...
3. **Postgres/S3 Backends:** `sulcus` uses a PostgreSQL-compatible local backend (PGlite bridge or Postgres). `sulcus-server` uses AWS RDS and S3 for infinite scale. The architecture itself is different.
...
2. **Make `sulcus` strictly single-player.** It writes to a file, not a socket.
...
| --------------- | ------------------- | ---------------- | ------------------------------------------------------------------------------ |
| **Community**   | **Free**            | Individual Devs  | The Open Source `sulcus` binary. BYO Storage (Local disk).               |
...
members = [
    ...
    "crates/sulcus",   # Open Source CLI (The Agent's Sidecar)
...
## 3. The Open Source CLI (`crates/sulcus`)
...
2. **Local MVP:** Build `sulcus` with PGlite/Postgres-compatible local storage and `fastembed`. Verify it works with OpenClaw via Stdio.
...
5. **Integration:** Configure `sulcus` to push data to `sulcus-server`.
...
members = [
    ...
    "crates/sulcus",   # The Sidecar (Open Source CLI)
...
## 2. The Local Sidecar (`crates/sulcus`)
...
_Used by OpenClaw/Cursor via `sulcus`._
